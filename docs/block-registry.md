...
- Bento(4×4) 레이아웃: DnD 종료 시 블록 배열 순서대로 4×4 그리드에 충돌 없는 위치(x, y)를 다시 계산하고, `save_block_layout` RPC(`p_page_id`, `p_blocks`)에 `{ id, x, y, w, h }[]`를 debounce 300ms로 전달합니다. w,h는 1~2, (x+w)≤4, (y+h)≤4를 만족하지 않으면 서버 CHECK에 막힙니다.
