...
- 블록 순서 변경은 DnD 종료 이벤트에서 `arrayMove`로 로컬 순서를 즉시 조정한 뒤 `{ id, ordering }` 배열을 `reorder_blocks_after_dnd` RPC(`p_page_id`, `p_blocks`)에 전달합니다. 실패 시 이전 순서로 롤백하고 저장 상태를 에러로 표시합니다.
