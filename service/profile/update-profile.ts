 ...
 */
export const updateProfile = async (params: UpdateProfileParams): Promise<UpdateProfileResult> => {
  ...
  try {
    ...
    await Sentry.startSpan({ op: "service.profile.update", name: "Update profile" }, async (span) => {
      if (normalizedUsername) {
        await client.users.updateUser(userId, { username: normalizedUsername });
        span.setAttribute("username.updated", true);
      }

      if (avatarFile instanceof File && avatarFile.size > 0) {
        await client.users.updateUserProfileImage(userId, { file: avatarFile });
